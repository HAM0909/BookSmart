...
def convert_availability(disponibilite):
    if pd.isna(disponibilite):
        return False   # valeur par défaut = False
    if isinstance(disponibilite, bool):
        return disponibilite
    if isinstance(disponibilite, str):
        return disponibilite.lower() in ['true', 'yes', '1', 'disponible']
    return bool(disponibilite)
...
def main():
    try:
        ...
        print("Lecture du fichier CSV brut...")
        df = pd.read_csv('livres_bruts.csv')
        print(f" Nombre de livres avant nettoyage: {len(df)}")
        ...
        # ✅ Sécurité : forcer la colonne en booléen
        df["disponibilite"] = df["disponibilite"].astype(bool)
...
def get_db():
    """
    Fournit une connexion PostgreSQL utilisable dans FastAPI
    """
    load_dotenv()
    DB_USER = os.getenv("DB_USER")
    DB_PASSWORD = os.getenv("DB_PASSWORD")
    ...
    try:
        ...
        print(f"[ERREUR] Connexion BD impossible: {str(e)}")
        raise e
