...
# App setup (corrigé : les guillemets manquaient)
app = FastAPI(title="Book Reservation API (Réserver un livre)")

# Corrigé : les chemins doivent être entre guillemets
app.mount("/static", StaticFiles(directory=".", html=True))

# Database setup (corrigé : os.getenv prend des chaînes)
...
@app.post("/api/reservations")   # corrigé : guillemets manquants
def reserver_un_livre(id_livre: int, id_adherent: int):
    conn = get_db()
    cur = conn.cursor()

    # corrigé : print doit être une chaîne formatée
    print("DEBUG >> trying to reserve:", id_livre, "for user:", id_adherent)

    try:
        # corrigé : requête SQL doit être entre guillemets
        ...
        # corrigé : stock est une clé donc ["stock"]
...
# Fonction vide en attente
def cancel_reservation(id_resa):
    pass

# corrigé : print avec guillemets
