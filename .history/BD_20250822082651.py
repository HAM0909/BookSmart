...
try:
    ...
    # Suppression de la partie création de base (inutile et potentiellement dangereuse)
    # La connexion SQLAlchemy ci-dessus vérifie déjà que la base existe
...
def recommander(titre, n=5):
    ...
    idx = data[data['titre'] == titre].index[0]  # Correction ici
    scores = list(enumerate(similarite[idx]))
    scores = sorted(scores, key=lambda x: x[1], reverse=True)
    scores = scores[1:n+1]  # Correction ici pour éviter d'inclure le livre lui-même
...
if len(data) > 0:
    exemple_titre = data['titre'].iloc[0]  # Correction ici - il manquait [0]
